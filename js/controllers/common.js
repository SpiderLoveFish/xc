...
(function(mui, common) {
	...
	aCommentList = function(commentType, masterID, startIndex, endIndex) {
		...
		common.postApi(interfaceUrl + 'EnterpriseApp/v1.0/GetAComment', data, function(response) {
			...
			"html":'',
			"js":'common',
			"url":interfaceUrl + 'EnterpriseApp/v1.0/GetAComment',
			"urldata":JSON.stringify(data),
			"returndata":JSON.stringify(response.data)
		}
		 common.postTraceApi(trace);
	...
	//	alert(interfaceName + JSON.stringify(data))
	//alert((userinfo.CorpId + '/' + userinfo.UserId + '/' + userinfo.ClientId + '/' + Math.round(new Date().getTime() / 1000)));
	...
	common.postTraceApi  = function(options) {
	mui.ajax('http://mb.xczs.co/webserver/AppApi.asmx/App_Trace',{
			data:options,
			dataType:'json',
			type:'post',
			timeout:1000,
