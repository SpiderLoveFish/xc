...
mui.plusReady(function() {
	...
	common.postApi('GetApprove', jsn, function(response) {
		...
		$('#pic').html(imgHtml);
		//业务数据
		$('#leaveType').text(obj.DictionaryName);
		$('#leaveTitle').text(obj.LTheme);
		$('#leaveContent').text(obj.LContext);
		$('#leaveStartTime').text(obj.StartTime);
		$('#leaveEndTime').text(obj.EndTime);
		$('#leaveDay').text(obj.LeaveDay + "天" + obj.LeaveHour + "小时");
		$('#leaveUserAvatar').attr('src', obj.Avatar);
		$('#leaveUserName').text(obj.UserName);
		$('#leaveTime').text(obj.ReleaseTime);
		if (obj.LSeatus == '2') {
			$('#approveState').addClass('approve_pass');
			$('#approveState').text('审批通过');
		} else if (obj.LSeatus == '3') {
	...
	function approve() {
		...
		if (status == "Pass") {
			...
			argu = "{'vid':'leave','pid':'1'}";
			pushTitle = getUserInfo().UserName + "申请了请假:" + leaveObj.LTheme;
			pushContent = leaveObj.LContext;
		} else {
			//点击消息跳转我发起的
			argu = "{'vid':'leave','pid':'0'}";
			pushTitle = "你申请的请假:" + leaveObj.LTheme;
		...
		var data = {
			...
			approveOrder: '',
			leaveType: '', //类型
			leaveTitle: leaveObj.LTheme, //请假单名称
			leaveContent: approveOpinion,
			pickDateBtn: '', //开始日期
			pickTimeBtn: '', //开始时间
			pickDateBtnEnd: '', //截止日期
			pickTimeBtnEnd: '', //截止时间
